...
var multiload = require('../multiload.js')
...
Apps.prototype.load = function (href) {
  ...
  boot.load(href, { load: self.loader }, function (err, vers) {
...
Apps.prototype.remove = function (app, cb) {
  var self = this
  cb = once(cb || noop)
  if (!self.boots[app.id]) return cb(new Error('no such id: ' + app.id))
  self.db.del('app-id!' + app.id, function (err) {
    if (err) return cb(err)
    delete self.loop.state.apps[app.id]
    delete self.boots[app.id]
    self.loop.extend({ apps: self.loop.state.apps })
    cb(null)
  })
}
...
Apps.prototype.update = function (app) {
  var self = this
  var boot = self.boots[app.id]
  app.hrefs.forEach(function (href) {
