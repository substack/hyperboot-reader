...
module.exports = function (loop, db) {
  ...
  loop.bus.on('help', function (topic) {
    loop.bus.emit('go', loop.state.url + '/help/' + topic)
  })
  loop.bus.on('close-help', function (topic) {
    loop.bus.emit('go', loop.state.url.replace(/\/help\/[^\/]*$/, ''))
  })
}
